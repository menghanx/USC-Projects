...
public class calibrate {
	...
	static final int calibrateMaxDepth = 12;
	...
	public static void main(String[] args) {	
		...
		benchMark(calibrateMaxDepth);	
		
	}

	private static void benchMark(int maxDepth) {
		...
		for (int d = 0; d <=maxDepth; d ++ ) {
			...
			int depthLimit = d;
			iterativeDeepeningSearch(earlyGameBoard, true, 100000, depthLimit);
		...
		for (int i = 0; i <=maxDepth; i++) {
			System.out.println("depth: " + i + " timetaken: " + benchMarkResults[i]);
		}
        ...
        for (char[] row: board)
            for (char c : row)
                if (c!='.')
                	totalPieces++;
        int depth = 11;
		long  timePerPlay = 3000;
        
//        System.out.println(Arrays.toString(calibrationResults));
		if (calibrationResults != null) {
			...
			// first 50 plays
			else if (playCounter > 1 && playCounter <= 50) {
				if (gameTimeLimit > 200000) {
					depth = 11;
					// use the calibration.txt maximum time limit for depth=14
					if (calibrationResults[calibrateMaxDepth] < 2000) {
						timePerPlay = 2500;
					}else {
						timePerPlay = calibrationResults[calibrateMaxDepth];
					}
				}else {
					timePerPlay = 1000;
				}
			...
			// playCouter > 50
				...
				if (gameTimeLimit < 200000 && gameTimeLimit >= 100000) {
					// time limit >
					depth = 8;
					timePerPlay =  calibrationResults[depth+4];
				}
				...
				else if (gameTimeLimit < 10000 && gameTimeLimit >= 1000) {
					depth = 4;
					timePerPlay = calibrationResults[depth+3];;
				}
			...
			// if calibration.txt is not there
			
		}

		playDepthLimit = depth;
		playTimeLimit = timePerPlay;
		System.out.println("playCounter:"+playCounter+" gameTimeLimit:"+gameTimeLimit +" playDepthLimit" + playDepthLimit +" playTimeLimit:"+playTimeLimit);
	}
